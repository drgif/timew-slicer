(ns user
  ...
  #_[lambdaisland.classpath.watch-deps :as watch-deps] ;; hot loading for deps
    [mwiederhold.timew-slicer.core :refer [start-app]]))
...
(defn dev-prep!
  ...
  (integrant.repl/set-prep! (fn []
                              (-> (mwiederhold.timew-slicer.config/system-config {:profile :dev})
                                  (ig/prep)))))

(defn test-prep!
  []
  (integrant.repl/set-prep! (fn []
                              (-> (mwiederhold.timew-slicer.config/system-config {:profile :test})
                                  (ig/prep)))))
